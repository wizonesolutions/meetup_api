<?php
    ...
    * Uses John Lim's AdoDB and phpxmlrpc libs
    *
    * @author Daniele Baroncelli
    * @author Gaetano Giunta
    * @copyright (c) 2003-2009 Giunta/Baroncelli. All rights reserved.
    *
    * @version $Id: toxmlrpc.inc.php 18 2009-06-23 12:57:01Z ggiunta $
    *
    * @todo some more error checking here and there
    * @todo document the xmlrpc-struct used to encode recordset info
    * @todo verify if using php_xmlrpc_encode($rs->GetArray()) would work with:
    *       - ADODB_FETCH_BOTH
    *       - null values
    * @todo add a faster method to extract data: xmlrpcval -> array
    */
    ...
    */
            ...
            if ($adodbrs->fetchMode == 'ADODB_FETCH_BOTH' || count($adodbrs->fields) == 2 * $adodbrs->FieldCount())
                for ($i = 0; $i < $numfields; $i++)
                    if ($adodbrs->fields[$i] === null)
                        ...
                        $columns[$i] = php_xmlrpc_encode ($adodbrs->fields[$i]);
            else
                foreach ($adodbrs->fields as $val)
                    if ($val === null)
                        $columns[] =& new xmlrpcval ('');
                    else
                        $columns[] = php_xmlrpc_encode ($val);
    ...
    /**
    * Returns an xmlrpc struct value as xml string out of an AdoDB recordset
    * @param ADORecordSet $adodbrs
    * @return string
    */
